...
function gameTick() {
  ...
  if (newHead.x === state.food.x && newHead.y === state.food.y) {
    ...
    state.score += 1;
    state.collectedPills.add(`${state.food.x},${state.food.y}`);
    state.food = findFood();
...
function handleDeath() {
  if (state.isGameOver || state.isDying) return;
  ...
  if (state.lives <= 0) {
    ...
    setStatus('ALL 5 LIVES USED - RESTART GAME');
    draw();
    showRestartDialog();
    return;
  }

  setStatus(`SNAKE TERMINATED - ${state.lives} ${state.lives === 1 ? 'life' : 'lives'} left`);
  draw();
...
function showRestartDialog() {
  const ok = window.confirm('All 5 lives are used. Restart current level?');
  if (ok) {
    state.isGameOver = false;
    state.lives = MAX_LIVES;
    state.eatenInLevel = 0;
    state.isDying = false;
    state.collectedPills = new Set();
    saveCurrentProgress();
    resetLevel();
  }
}
...
function drawBoard() {
  ...
  state.collectedPills.forEach((key) => {
    const [x, y] = key.split(',').map(Number);
    drawCell(x, y, '#f59e0b', '#b45309');
  });
...
if (mobileInput) {
  ...
  saveProgressBtn.addEventListener('click', () => {
    const mobileNumber = getCurrentMobileNumber();
    if (!mobileNumber) {
      setStatus('Enter mobile number to login');
      return;
    }
    saveCurrentProgress();
  });
  ...
  logoutBtn.addEventListener('click', () => {
    const mobileNumber = getCurrentMobileNumber();
    if (mobileNumber) {
      localStorage.removeItem(buildProgressKey(mobileNumber));
    }
    localStorage.removeItem(LAST_MOBILE_KEY);
    updateAuthUI();
